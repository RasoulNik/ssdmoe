...
4. Added `src/streaming_moe/expert_store.py` to read experts by byte offset with `pread()`.
...
New files:
- `src/streaming_moe/pipelined_moe.py`
...
- `src/streaming_moe/server/http.py`
- `src/streaming_moe/server/runtime_adapter.py`
- `src/streaming_moe/server/protocol.py`
...
- added lightweight expert-selection tracing in:
  - `src/streaming_moe/streamed_switch.py`
- added study harness:
...
- `src/streaming_moe/native_reader.py` — added ctypes binding
- `src/streaming_moe/expert_store.py` — `read_components_batched` now calls the
  single-call C path
...
- `src/streaming_moe/native_reader.py` — `NativeSlab`, ctypes bindings for all three
- `src/streaming_moe/session_window_cache.py` — `SessionState.expert_lut`, slab-based `LayerWindow`, rewritten `load_components_for_layer`
...
**Fix.**  `src/streaming_moe/fused_expert.py` provides `compute_expert_output_fused`
...
- `src/streaming_moe/fused_expert.py` — `fused_gate_up_swiglu`, `compute_expert_output_fused`
- `src/streaming_moe/streamed_switch.py` — `fused_gate_up` / `compile_fused_gate_up` wiring
- `src/streaming_moe/pipelined_moe.py`, `prefetch_switch.py` — same wiring
...
- `src/streaming_moe/server/protocol.py` — added `parse_tool_calls()` (XML regex parser),
  ...
  `tool_calls=`
- `src/streaming_moe/server/http.py` — `ServerCapabilities(tools=True)`, `-nothink` model
...
- `src/streaming_moe/server/persistent_cache.py` (new) — `PersistentPromptCache` wrapping
  `LRUPromptCache` with async safetensors save, `load_from_disk()`, `_evict_disk()`
- `src/streaming_moe/server/runtime_adapter.py` — replaced `LRUPromptCache` with
  `PersistentPromptCache`, added `load_from_disk()` in `warmup()`, added `close()`
- `src/streaming_moe/server/http.py` — added `--kv-cache-dir` (default `.run/kv-cache`)
